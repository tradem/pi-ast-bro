# ast-output-budgeting Specification

## Purpose
Enforce a consistent, configurable output-size ceiling on `analyze_ast_search` snippets mode so that relevance-ranked results are bounded without overriding the agent's explicit `top_k`.
